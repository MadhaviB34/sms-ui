...
export default{
    props:['myclass','school','userdetails','acdemicyearandboard','assignments'],
        ...
        newassignment:function(){
            ...
            UserService.AddMySubjectAssignment(assignment,this.requestheaders,this.id).then((response)=>{
                if(response.status==200){
                   this.$router.push('/myclass/'+this.myclass.gradenumber+this.myclass.sectionname+'-'+this.myclass.syllabustype+'/assignments');
                }
            }).catch((response)=>{
