...
export default{
    props:['mysubject','school','userdetails','acdemicyearandboard','assignments'],
    ...
    methods:{
        newassignment:function(){
            ...
            UserService.AddMySubjectAssignment(assignment,this.requestheaders,this.id).then((response)=>{
                if(response.status==200){
                   this.$router.push('/mysubjects/'+this.mysubject.gradenumber+this.mysubject.sectionname+'-'+this.mysubject.subjectname+'-'+this.mysubject.syllabustype+'/assignments');
                }
            }).catch((response)=>{
